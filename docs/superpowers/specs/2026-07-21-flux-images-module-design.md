...
- **Rủi ro phần cứng — ĐÃ chạy smoke test thật (2026-07-21), kết quả: pipeline code đúng, bị chặn bởi hạ tầng máy, không phải bug MPS/MLX**:
  1. `mflux-generate` yêu cầu xác thực Hugging Face — `black-forest-labs/FLUX.1-schnell` là gated repo (phải đăng nhập HF, chấp nhận điều khoản model, tạo `HF_TOKEN` và thêm vào `.env`) dù code license là Apache-2.0. Đã ghi vào global constraints/README vận hành.
  2. Backend tải file nhanh "xet" của `huggingface_hub` bị lỗi (`RuntimeError: Unable to parse string as hex hash value`) khi tải repo này — đã cố định `HF_HUB_DISABLE_XET=1` trong `images/generator.py` (không cần người vận hành tự set).
  3. **Máy hiện tại không đủ dung lượng ổ đĩa**: model FLUX.1-schnell cần tải ~10GB, máy chỉ còn ~750MB trống (đã dùng 95% ổ 228GB) → tải dở dang rồi lỗi `OSError: No space left on device`. Đây là rủi ro hạ tầng thật của máy cụ thể, không phải lỗi code — **cần dọn ít nhất ~15GB trống trước khi thử lại**.
  4. Chưa xác nhận được bước sinh ảnh thực tế (sau khi tải model xong) có chạy ổn định trên MLX hay không — quá trình test bị chặn ở bước tải model do thiếu dung lượng, chưa tới được bước inference thật. Vẫn cần chạy lại sau khi có đủ dung lượng để xác nhận trọn vẹn mục "Rủi ro phần cứng" này.
