...
Chains this repo's five pipeline modules — `scripts` → `tts` → `images` → `video` → `metadata` — into one finished MP4. Each module is a CLI that reads the previous module's JSON output and writes its own JSON + asset output; there is no single orchestrator command, so each stage's exact output path must be captured and passed to the next.
